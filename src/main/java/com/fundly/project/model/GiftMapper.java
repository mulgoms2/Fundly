...
public interface GiftMapper {
    ...
    int insert(@Param("GiftDto") GiftDto giftDto) throws Exception;
    // 선물 하나 등록하기
    ...
    List<GiftDto> selectByStatus(@Param("GiftDto") GiftDto giftDto) throws Exception;
    // 특정 프로젝트의 판매상태에 따른 선물 목록 가져오기(ex. 프로젝트1의 판매중 선물 가져오기)
    int updateContentBefore(@Param("GiftDto") GiftDto giftDto) throws Exception;
    // 해당 선물의 상세 내역 변경(프로젝트 등록중)
    int updateStatus(@Param("GiftDto") GiftDto giftDto) throws Exception;
    // 해당 선물의 판매 상태 변경(판매중, 일시품절)
    int updateQty(@Param("GiftDto") GiftDto giftDto) throws Exception;
    // 해당 선물의 판매 수량 수정
