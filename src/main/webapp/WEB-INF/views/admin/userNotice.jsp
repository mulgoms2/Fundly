        ...
        <div class="Container__ContainerComponent-o2c7wa-0 dwqwNI StyledTBB__column2Wrapper-trchgn-2 CommonStyled__Wrapper-bpcmiq-0 List__StyledWrapper-sc-172nbbl-2 gYkBev cwoqcI kctMgq">
            <div class="list-wrapper">
                <div class="List__StyledList-sc-1ldfygd-0 cwiHPf">
                    ...
                    <li class>
                        <a href="<c:url value='/noticeDetail?news_seq=${NewsDto.news_seq}&page=${page}'/>">
                                <span class="title-box">
                                    <span class="group">공지사항
                                        <em class="empty"></em>
                                    </span>
                                    <span class="title"> ${NewsDto.news_title}</span>
                                    <span class="date">${NewsDto.mod_dtm!=null? NewsDto.mod_dtm :NewsDto.reg_dtm}</span>
                                </span>
                            <div class="img-box">
                                <img src="">
                            </div>
            ...
            <div class="pg" style=" display: flex;  margin: 20px 400px 40px;justify-content: space-around;">
                <c:if test="${ph.showPrev}">
                    <a href="<c:url value='/noticeList?page=${ph.beginPage-1}'/>">&lt;</a>
                </c:if>
                <c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
                    <a href="<c:url value='/noticeList?page=${i}'/> ">${i}</a>
                </c:forEach>
                <c:if test="${ph.showNext}">
                    <a href="<c:url value='/noticeList?page=${ph.endPage+1}'/>">&gt;</a>
                </c:if>
            </div>
