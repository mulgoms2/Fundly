...
class ProjectBasicInfoControllerTest {
    ...
    void setUp() throws JsonProcessingException {
        mockMvc = MockMvcBuilders.standaloneSetup(projectBasicInfoController)
                                 .build();
        objectMapper = new ObjectMapper();

        pj_id = "01";
        user_id = "mulgom";
        request = ProjectInfoUpdateRequest.builder()
                                          .pj_id(pj_id)
                                          .build();
        response = ProjectInfoUpdateResponse.builder()
                                            .pj_id(pj_id)
                                            .build();
...
//    }
    ...
    void get_start_with_project() throws Exception {
        ProjectStarter pjStarter = ProjectStarter.builder()
                                                 .build();
...
//    }
    ...
    void getEditingProject() throws Exception {
        given(service.getEditingProject(any())).willReturn(ProjectDto.builder()
                                                                     .pj_id(pj_id)
                                                                     .build());
    ...
    void makeProject() throws Exception {
        ProjectAddRequest addRequest = ProjectAddRequest.builder()
                                                        .user_email(user_id)
                                                        .build();
    ...
    void new_project_success() throws Exception {
        ...
        mockMvc.perform(post(BASE_URL + "/info").param("user_email", user_id)
                                                .sessionAttr("user_email", user_id))
               .andExpect(status().is3xxRedirection())
    ...
    void sessionCheck() throws Exception {
        ...
        mockMvc.perform(post(BASE_URL + "/info").param("user_email", "dbswo")
                                                .sessionAttr("user_email", user_id))
               .andExpect(status().is3xxRedirection())
               .andExpect(rm)
               .andDo(print());
    ...
    void 업데이트실패() throws Exception {
        ...
        ProjectInfoUpdateRequest updateRequest = ProjectInfoUpdateRequest.builder()
                                                                         .pj_id("01")
                                                                         .build();
...
//        given(FileUploader.uploadFile(any())).willThrow(ImageSaveFailureException.class);
        ...
        mockMvc.perform(multipart("/project/editor/info/image").file(file)
                                                               .sessionAttr("user_email", "dbswo"))
               .andDo(print());
